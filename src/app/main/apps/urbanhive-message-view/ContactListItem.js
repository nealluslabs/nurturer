...
function ContactListItem(props) {
 ...
 let matchedUser = messageInFocus/*filteredContacts.filter((item)=>(item.uid === props.user.uid)) && filteredContacts.filter((item)=>(item.uid === props.user.uid))[0]*/
console.log("WHAT IS MATCHED USER ---->", matchedUser)


//NO NEED TO FILTER AS ITS JUST ONE MESSAGE ON THIS SCREEN
 let onlyPendingMessages = matchedUser?.messageQueue  && matchedUser?.messageQueue.filter((item)=>(item))
...
onlyPendingMessages = matchedUser?.messageQueue  && matchedUser?.messageQueue.filter((item)=>(item))
